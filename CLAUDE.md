...
- `indicators/mmcMarketMagnet.js` — port of the ThinkorSwim "Market Magnet PSR" (Million-Dollar-Margin Club), which is really a session volume profile. Accumulates `d.profile()` across the NY session (keyed by tick), then overlays the POC ("magnet", magenta) and the 50% value-area bands VAH/VAL (green/red dashed), re-anchoring each session. Reuses the ET/DST session helpers from `nySessionAtrLevels.js`; falls back to typical-price×volume when a bar has no profile.
